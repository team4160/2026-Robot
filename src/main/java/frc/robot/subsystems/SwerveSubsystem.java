...
public class SwerveSubsystem extends SubsystemBase {
	...
	public void setupPathPlanner() {
		// Load the RobotConfig from the GUI settings. You should probably store this in your DrivebaseConstants file
		RobotConfig config;
		try {
		...
		return run(() -> {
			...
			if (resultO.isPresent()) {
				...
				if (result.hasTargets()) {
					drive(getTargetSpeeds(0, 0, Rotation2d.fromDegrees(result.getBestTarget().getYaw()))); // Not sure if this will work, more math may be
					// required.
				}
			}
		});
